...
export default class Listings {
    ...
    checkByPriceKey({
        ...
        const autoGenerated = match?.group === 'invalidItem' || match?.group === 'painted';
        const unreservedStock = isAssetId
            ? Number(!this.bot.handler.isCraftingAssetReserved(priceKey))
            : (sku ? inventory.findBySKU(sku, true) : []).filter(id => !this.bot.handler.isCraftingAssetReserved(id)).length;
        const amountCanSell = !isRobotPartSku(sku) && autoGenerated
            ? 0
            : Math.min(unreservedStock, invManager.amountCanTrade({
                  priceKey,
                  tradeIntent: 'selling',
                  getGenericAmount: checkGenerics
              }));
        ...
        listings.forEach(listing => {
            if (listing.intent === 1 && this.bot.handler.isCraftingAssetReserved(listing.id.replace('440_', ''))) {
                log.warn(`Removing sell listing for reserved customer asset ${listing.id}`);
            ...
            if (isAssetId && null !== inventory.findByAssetid(priceKey)) {
                ...
                assetids = foundAssetIds.filter(
                    assetId => this.bot.pricelist.hasPrice({ priceKey: assetId, onlyEnabled: false }) === false &&
                        !this.bot.handler.isCraftingAssetReserved(assetId)
                );
    ...
    checkAll(): Promise<void> {
        return new Promise(resolve => {
            ...
            const doneRemovingAll = (): void => {
                ...
                // PriceDB Store has its own persistent listings; backpack.tf removeAll does not clear them.
                for (const listing of this.bot.pricedbStoreManager?.getAllListings() ?? []) {
                    if (this.bot.handler.isCraftingAssetReserved(listing.asset_id)) { void this.deletePriceDBListing(listing.asset_id); continue; }
                    const sku = inventory.findByAssetid(listing.asset_id);
                    const entry = this.bot.pricelist.getPrice({ priceKey: listing.asset_id, onlyEnabled: false })
                        ?? (sku ? this.bot.pricelist.getPrice({ priceKey: sku, onlyEnabled: false }) : null);
                    if (!sku || (!isRobotPartSku(sku) &&
                        (!entry || entry.group === 'invalidItem' || entry.group === 'painted'))) {
                        log.warn(`Removing automatically listed non-robot item from PriceDB Store: ${listing.asset_id}`);
                        void this.deletePriceDBListing(listing.asset_id);
                    }
                }
    ...
    async createOrUpdatePriceDBListing(assetId: string, currencies: Currencies): Promise<void> {
        ...
        const sku = this.bot.inventoryManager.getInventory.findByAssetid(assetId);
        if (this.bot.handler.isCraftingAssetReserved(assetId)) {
            log.warn(`Refusing PriceDB Store listing for customer asset ${assetId}`);
            return;
        }
        const entry = this.bot.pricelist.getPrice({ priceKey: assetId, onlyEnabled: false })
            ?? (sku ? this.bot.pricelist.getPrice({ priceKey: sku, onlyEnabled: false }) : null);
        if (!sku || (!isRobotPartSku(sku) &&
            (!entry || entry.group === 'invalidItem' || entry.group === 'painted'))) {
            log.warn(`Refusing automatic PriceDB Store listing for non-robot item ${assetId}`);
            return;
        }
